...
cursor = conn.cursor()
if (cursor):
	cursor.execute('SELECT tSchematic.schematicID, schematicName, objectPath, imageName FROM tSchematic LEFT JOIN (SELECT schematicID, imageName FROM tSchematicImages WHERE imageType=1) tsi ON tSchematic.schematicID=tsi.schematicID WHERE objectPath IN (SELECT ingredientObject FROM tSchematicIngredients WHERE ingredientType>0)' + criteriaStr + ' ORDER BY schematicName;')
	row = cursor.fetchone()
