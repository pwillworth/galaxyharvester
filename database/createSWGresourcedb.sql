...
CREATE TABLE tResourceTypeCreature (resourceType VARCHAR(63), speciesName VARCHAR(63), maxAmount SMALLINT, PRIMARY KEY (resourceType, speciesName));
...
CREATE TABLE tSchematicEvents (galaxy SMALLINT, eventTime DATETIME, eventType CHAR(1), schematicID VARCHAR(127), expGroup VARCHAR(31), spawnID INT, eventDetail VARCHAR(1023), PRIMARY KEY (spawnID, schematicID, expGroup), INDEX IX_schemevt_galaxy (galaxy), INDEX IX_schemevt_galaxy_eventtime (galaxy, eventTime), INDEX IX_schemevt_galaxy_schem (galaxy, schematicID), INDEX IX_schemevt_spawn (spawnID));
CREATE TABLE tGalaxy (galaxyID INT AUTO_INCREMENT PRIMARY KEY, galaxyName VARCHAR(127), lastExport DATETIME, galaxyState SMALLINT DEFAULT 0 NOT NULL, totalSpawns INT, totalWaypoints INT);
...
CREATE TABLE tProfession (profID SMALLINT AUTO_INCREMENT PRIMARY KEY, profName VARCHAR(31), craftingQuality SMALLINT);
CREATE TABLE tSkillGroup (profID SMALLINT, skillGroup VARCHAR(31), skillGroupName VARCHAR(63), INDEX IX_profsg_prof (profID), INDEX IX_profsg_sg (skillGroup), INDEX IX_profsg_sgname (skillGroupName));
CREATE TABLE tSchematic (schematicID VARCHAR(127) PRIMARY KEY, schematicName VARCHAR(255), craftingTab INT, skillGroup VARCHAR(31), objectType INT, complexity SMALLINT, objectSize SMALLINT, xpType VARCHAR(31), xpAmount SMALLINT, objectPath VARCHAR(255), objectGroup VARCHAR(255), INDEX IX_schem_skill (skillGroup));
...
LOAD DATA LOCAL INFILE '/var/www/database/seedData/professions.csv' INTO TABLE tProfession FIELDS ENCLOSED BY '"' (profName);
